...
if (isset($_REQUEST['add_ban']) || isset($_GET['edit_ban'])) {
	if (isset($_GET['add_ban']) || isset($_POST['add_ban'])) {
		...
		if (isset($user_id)) {
			$result = $db->query('SELECT commenter_ip FROM '.$db->prefix.'comments WHERE commenter_id='.$user_id.' ORDER BY posted DESC LIMIT 1') or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
