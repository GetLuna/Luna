...
elseif (isset($_GET['del_forum'])) {
	...
	if (isset($_POST['del_forum_comply'])) { // Delete a forum with all comments
		...
		$result = $db->query('SELECT t1.id FROM '.$db->prefix.'threads AS t1 LEFT JOIN '.$db->prefix.'threads AS t2 ON t1.moved_to=t2.id WHERE t2.id IS NULL AND t1.moved_to IS NOT NULL') or error('Unable to fetch redirect topics', __FILE__, __LINE__, $db->error());
		...
		if ($num_orphans) {
			...
			$db->query('DELETE FROM '.$db->prefix.'threads WHERE id IN('.implode(',', $orphans).')') or error('Unable to delete redirect topics', __FILE__, __LINE__, $db->error());
		}
	...
	if (isset($_POST['save'])) {
		...
		if (isset($_POST['read_forum_old'])) {
			...
			while ($cur_group = $db->fetch_assoc($result)) {
				...
				$create_topics_new = isset($_POST['create_topics_new'][$cur_group['g_id']]) ? '1' : '0';

				// Check if the new settings differ from the old
				if ($read_forum_new != $_POST['read_forum_old'][$cur_group['g_id']] || $comment_new != $_POST['comment_old'][$cur_group['g_id']] || $create_topics_new != $_POST['create_topics_old'][$cur_group['g_id']]) {
					// If the new settings are identical to the default settings for this group, delete its row in forum_perms
					if ($read_forum_new == '1' && $comment_new == $cur_group['g_comment'] && $create_topics_new == $cur_group['g_create_threads'])
						...
						$db->query('UPDATE '.$db->prefix.'forum_perms SET read_forum='.$read_forum_new.', comment='.$comment_new.', create_topics='.$create_topics_new.' WHERE group_id='.$cur_group['g_id'].' AND forum_id='.$forum_id) or error('Unable to insert group forum permissions', __FILE__, __LINE__, $db->error());
						if (!$db->affected_rows())
							$db->query('INSERT INTO '.$db->prefix.'forum_perms (group_id, forum_id, read_forum, comment, create_topics) VALUES('.$cur_group['g_id'].', '.$forum_id.', '.$read_forum_new.', '.$comment_new.', '.$create_topics_new.')') or error('Unable to insert group forum permissions', __FILE__, __LINE__, $db->error());
					}
				}
			}
		}
...
<?php
	...
	$result = $db->query('SELECT g.g_id, g.g_title, g.g_read_board, g.g_comment, g.g_create_threads, fp.read_forum, fp.comment, fp.create_topics FROM '.$db->prefix.'groups AS g LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (g.g_id=fp.group_id AND fp.forum_id='.$forum_id.') WHERE g.g_id!='.LUNA_ADMIN.' ORDER BY g.g_id') or error('Unable to fetch group forum permission list', __FILE__, __LINE__, $db->error());
	...
	while ($cur_perm = $db->fetch_assoc($result)) {
		...
		$create_topics = (($cur_perm['g_create_threads'] == '0' && $cur_perm['create_topics'] == '1') || ($cur_perm['g_create_threads'] == '1' && $cur_perm['create_topics'] != '0')) ? true : false;
		...
		$create_topics_def = (($create_topics && $cur_perm['g_create_threads'] == '0') || (!$create_topics && ($cur_perm['g_create_threads'] == '' || $cur_perm['g_create_threads'] == '1'))) ? false : true;
...
?>
					<tr>
						...
						<td<?php if (!$create_topics_def) echo ' class="danger"'; ?>>
							<input type="hidden" name="create_topics_old[<?php echo $cur_perm['g_id'] ?>]" value="<?php echo ($create_topics) ? '1' : '0'; ?>" />
							<input type="checkbox" name="create_topics_new[<?php echo $cur_perm['g_id'] ?>]" value="1"<?php echo ($create_topics) ? ' checked' : ''; ?> tabindex="<?php echo $cur_index++ ?>" />
						</td>
					</tr>
<?php
