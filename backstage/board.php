                            ...
                            <div class="col-md-9">
                                <div class="input-group">
                                    ...
                                    <div class="input-group-prepend">
                                        <span class="input-group-text" id="inputGroupPrepend">fa-fw fa-</span>
                                    </div>
...
<div class="row">
    <div class="col">
        <form class="card" method="post" action="board.php">
...
$result = $db->query('SELECT id, cat_name FROM '.$db->prefix.'categories ORDER BY disp_position') or error('Unable to fetch category list', __FILE__, __LINE__, $db->error());
    if ($db->num_rows($result) > 0) {
        ?>
			<div class="card">
				<h5 class="card-header">
                    <?php _e('Add forum', 'luna')?>
                    <span class="float-right">
                        <button class="btn btn-link" type="submit" name="add_forum" tabindex="2"><span class="fas fa-fw fa-plus"></span> <?php _e('Add', 'luna')?></button>
                    </span>
                </h5>
                <div class="card-body">
...
?>
		<form method="post" action="board.php">
			<div class="card">
				<h5 class="card-header">
                    <?php _e('Add categories', 'luna')?>
                    <span class="float-right">
                        <button class="btn btn-link" type="submit" name="add_cat" tabindex="2"><span class="fas fa-fw fa-plus"></span> <?php _e('Add', 'luna')?></button>
                    </span>
				</h5>
                <div class="card-body">
...
<?php
            ...
            while ($cur_category = $db->fetch_assoc($category)) {
                ?>
					<div class="title">
						<div class="title-lg title-primary">
						...
						<div class="title-md title-primary">
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text" id="inputGroupPrepend"><?php _e('Position', 'luna') ?></span>
                                </div>
...
$forum = $db->query('SELECT id, forum_name, disp_position, color, icon, icon_style FROM '.$db->prefix.'forums WHERE cat_id = '.$cur_category['id'].' AND parent_id = \'\' ORDER BY disp_position, id') or error('Unable to fetch forums', __FILE__, __LINE__, $db->error());
                ...
                while ($cur_forum = $db->fetch_assoc($forum)) {
                    ?>
					<div class="card card-board" style="border-color: <?php echo $cur_forum['color'] ?>">
						<h5 class="card-header" style="background: <?php echo $cur_forum['color'] ?>; border-color: <?php echo $cur_forum['color'] ?>;">
                            ...
                            <span class="float-right">
                                <a class="btn btn-link" href="board.php?edit_forum=<?php echo $cur_forum['id'] ?>" tabindex="<?php echo $cur_index++ ?>"><span class="fas fa-fw fa-edit"></span> <?php _e('Edit', 'luna')?></a>
                            </span>
                        </h5>
...
$subforum = $db->query('SELECT id, forum_name, disp_position, color, icon, icon_style, parent_id FROM '.$db->prefix.'forums WHERE cat_id = '.$cur_category['id'].' AND parent_id = '.$cur_forum['id'].' ORDER BY disp_position, id') or error('Unable to fetch forums', __FILE__, __LINE__, $db->error());
                    ...
                    while ($cur_subforum = $db->fetch_assoc($subforum)) {
                        ?>
                        <div class="card card-board card-sub" style="border-color: <?php echo $cur_subforum['color'] ?>">
                            <h5 class="card-header" style="background: <?php echo $cur_subforum['color'] ?>; border-color: <?php echo $cur_subforum['color'] ?>;">
                                ...
                                <span class="float-right">
                                    <a class="btn btn-link" href="board.php?edit_forum=<?php echo $cur_subforum['id'] ?>" tabindex="<?php echo $cur_index++ ?>"><span class="fas fa-fw fa-edit"></span> <?php _e('Edit', 'luna')?></a>
                                </span>
                            </h5>
