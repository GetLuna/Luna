		...
		if (isset($_POST['read_forum_old'])) {
			...
			while ($cur_group = $db->fetch_assoc($result)) {
				...
				if ($read_forum_new != $_POST['read_forum_old'][$cur_group['g_id']] || $post_replies_new != $_POST['post_replies_old'][$cur_group['g_id']] || $post_topics_new != $_POST['post_topics_old'][$cur_group['g_id']]) {
					...
					if ($read_forum_new == '1' && $post_replies_new == $cur_group['g_comment'] && $post_topics_new == $cur_group['g_create_threads'])
...
<?php
	...
	$result = $db->query('SELECT g.g_id, g.g_title, g.g_read_board, g.g_comment, g.g_create_threads, fp.read_forum, fp.post_replies, fp.post_topics FROM '.$db->prefix.'groups AS g LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (g.g_id=fp.group_id AND fp.forum_id='.$forum_id.') WHERE g.g_id!='.LUNA_ADMIN.' ORDER BY g.g_id') or error('Unable to fetch group forum permission list', __FILE__, __LINE__, $db->error());
	...
	while ($cur_perm = $db->fetch_assoc($result)) {
		...
		$post_topics = (($cur_perm['g_create_threads'] == '0' && $cur_perm['post_topics'] == '1') || ($cur_perm['g_create_threads'] == '1' && $cur_perm['post_topics'] != '0')) ? true : false;
		...
		$post_topics_def = (($post_topics && $cur_perm['g_create_threads'] == '0') || (!$post_topics && ($cur_perm['g_create_threads'] == '' || $cur_perm['g_create_threads'] == '1'))) ? false : true;
