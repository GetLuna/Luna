	...
	<?php endif; endif; ?>
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<div class="checkbox">
							<label>
								<input type="checkbox" name="post_replies" value="1"<?php if ($group['g_comment'] == '1') echo ' checked' ?> tabindex="19" />
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<div class="checkbox">
							<label>
								<input type="checkbox" name="post_topics" value="1"<?php if ($group['g_post_threads'] == '1') echo ' checked' ?> tabindex="21" />
...
elseif (isset($_POST['add_edit_group'])) {
	...
	if ($_POST['group_id'] != LUNA_ADMIN) {
		...
		$mod_edit_users = $mod_rename_users = $mod_change_passwords = $mod_ban_users = $read_board = $view_users = $post_replies = $post_topics = $edit_posts = $delete_posts = $delete_threads = $set_title = $search = $search_users = $send_email = $soft_delete_view = $soft_delete_posts = $soft_delete_threads = $inbox_allow = '1';
	...
	if ($_POST['mode'] == 'add') {
		...
		$db->query('INSERT INTO '.$db->prefix.'groups (g_title, g_user_title, g_moderator, g_mod_edit_users, g_mod_rename_users, g_mod_change_passwords, g_mod_ban_users, g_read_board, g_view_users, g_comment, g_post_threads, g_edit_comments, g_delete_comments, g_delete_threads, g_set_title, g_search, g_search_users, g_send_email, g_post_flood, g_search_flood, g_email_flood, g_report_flood, g_inbox, g_inbox_limit, g_soft_delete_view, g_soft_delete_comments, g_soft_delete_threads) VALUES(\''.$db->escape($title).'\', '.$user_title.', '.$moderator.', '.$mod_edit_users.', '.$mod_rename_users.', '.$mod_change_passwords.', '.$mod_ban_users.', '.$read_board.', '.$view_users.', '.$post_replies.', '.$post_topics.', '.$edit_posts.', '.$delete_posts.', '.$delete_threads.', '.$set_title.', '.$search.', '.$search_users.', '.$send_email.', '.$post_flood.', '.$search_flood.', '.$email_flood.', '.$report_flood.', '.$inbox_allow.', '.$inbox_limit.', '.$soft_delete_view.', '.$soft_delete_posts.', '.$soft_delete_threads.')') or error('Unable to add group', __FILE__, __LINE__, $db->error());
