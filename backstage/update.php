...
if ($luna_config['o_update_ring'] != 3) {
    if (version_compare(Version::LUNA_CORE_VERSION, $update_cache, 'lt')) {
        ?>
				...
				<div class="btn-group">
					<a href="http://getluna.org/cnt/get.php?id=4" class="btn btn-primary"><i class="fas fa-fw fa-download"></i> <?php echo sprintf(__('Download v%s', 'luna'), $update_cache) ?></a>
					<a href="http://getluna.org/release-notes.php" class="btn btn-primary"><i class="fas fa-fw fa-refresh"></i> <?php _e('Changelog', 'luna')?></a>
				</div>
<?php
...
<?php
    ...
    ?>
				...
				<p><?php _e('At this point, we can only tell you that you\'re beyond the latest release. We can\'t tell you if there is a new preview available. You\'ll have to find out for yourself.', 'luna')?></p>
				<div class="btn-group">
					<a href="http://getluna.org/cnt/get.php?id=4" class="btn btn-primary"><i class="fas fa-fw fa-download"></i> <?php _e('Download', 'luna')?></a>
				</div>
<?php }?>
			</div>
		</div>
