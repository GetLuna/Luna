...
if (isset($_POST['form_sent'])) {
	...
	if (empty($errors) && !isset($_POST['preview'])) {
		...
		// Update the comment
		$db->query('UPDATE '.$db->prefix.'comments SET message=\''.$db->escape($message).'\', admin_note=\''.$db->escape($admin_note).'\' WHERE id='.$id) or error('Unable to update comment', __FILE__, __LINE__, $db->error());
