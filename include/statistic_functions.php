    ...
    if ($luna_config['o_users_online'] == '1') {
        ...
        if ($db->num_rows($result) > 0) {
            ...
            while ($luna_user_online = $db->fetch_assoc($result)) {
                if ($luna_user['g_view_users'] == '1') {
                    echo '<li><a href="profile.php?id='.$luna_user_online['user_id'].'">'.luna_htmlspecialchars($luna_user_online['ident']).'</a></li>';
                } else {
                    echo '<li>'.luna_htmlspecialchars($luna_user_online['ident']).'</li>';
                }
