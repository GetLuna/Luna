...
// Make sure we have built in support for MariaDB
if (!function_exists('mariadb_connect'))
	exit('This PHP environment doesn\'t have MariaDB support built in. MariaDB support is required if you want to use a MariaDB database to run this forum. Consult the PHP documentation for further assistance.');
...
class DBLayer
{
	...
	function DBLayer($db_host, $db_username, $db_password, $db_name, $db_prefix, $p_connect)
	{
		...
		if ($p_connect)
			$this->link_id = @mariadb_pconnect($db_host, $db_username, $db_password);
		else
			$this->link_id = @mariadb_connect($db_host, $db_username, $db_password);

		if ($this->link_id)
		{
			if (!@mariadb_select_db($db_name, $this->link_id))
				error('Unable to select database. MariaDB reported: '.mariadb_error(), __FILE__, __LINE__);
		}
		else
			error('Unable to connect to MariaDB server. MariaDB reported: '.mariadb_error(), __FILE__, __LINE__);
	...
	function query($sql, $unbuffered = false)
	{
		...
		if ($unbuffered)
			$this->query_result = @mariadb_unbuffered_query($sql, $this->link_id);
		else
			$this->query_result = @mariadb_query($sql, $this->link_id);
		...
		else
		{
			...
			$this->error_no = @mariadb_errno($this->link_id);
			$this->error_msg = @mariadb_error($this->link_id);
	...
	{
		return ($query_id) ? @mariadb_result($query_id, $row, $col) : false;
	}


	function fetch_assoc($query_id = 0)
	{
		return ($query_id) ? @mariadb_fetch_assoc($query_id) : false;
	}


	function fetch_row($query_id = 0)
	{
		return ($query_id) ? @mariadb_fetch_row($query_id) : false;
	}


	function num_rows($query_id = 0)
	{
		return ($query_id) ? @mariadb_num_rows($query_id) : false;
	}


	function affected_rows()
	{
		return ($this->link_id) ? @mariadb_affected_rows($this->link_id) : false;
	}


	function insert_id()
	{
		return ($this->link_id) ? @mariadb_insert_id($this->link_id) : false;
	}
	...
	{
		return ($query_id) ? @mariadb_free_result($query_id) : false;
	}
	...
	function escape($str)
	{
		if (is_array($str))
			return '';
		else if (function_exists('mariadb_real_escape_string'))
			return mariadb_real_escape_string($str, $this->link_id);
		else
			return mariadb_escape_string($str);
	}
	...
	function close()
	{
		...
		{
			if ($this->query_result)
				@mariadb_free_result($this->query_result);

			return @mariadb_close($this->link_id);
		}
		else
			return false;
	}
