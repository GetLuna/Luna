...
if ($num_not_unseen == '0') {
    ...
    while ($cur_notifi = $db->fetch_assoc($result)) {
        ...
        $ind_not[] = '<div class="row notification-row">
                        <div class="col-10">
                        ...
                        <div class="col-2">
                            <a class="btn btn-primary btn-block" href="notifications.php?read_notification='.$cur_notifi['id'].'"><span class="fas fa-fw fa-eye"></span><span class="d-none d-sm-inline"> '.__('Seen', 'luna').'</span></a>
                        </div>
                    </div>';
    }
}
...
if ($num_not_seen == '0') {
    ...
    while ($cur_notifi = $db->fetch_assoc($result)) {
        $notifitime = format_time($cur_notifi['time'], false, null, $luna_config['o_time_format'], true, true);
        $ind_not_seen[] = '<div class="row notification-row">
                            <div class="col-10">
                                <a class="btn btn-light btn-block" href="notifications.php?notification='.$cur_notifi['id'].'"><span class="fas fa-fw '.$cur_notifi['icon'].'"></span>&nbsp; '.$cur_notifi['message'].'<span class="timestamp float-right d-none d-sm-inline">'.format_time($cur_notifi['time'], false, null, $luna_config['o_time_format'], true, true).'</span></a>
                            </div>
                            <div class="col-2">
