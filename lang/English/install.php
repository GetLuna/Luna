...
'Install message'				=>	'The file \'config.php\' doesn\'t exist or is corrupt. This implies that ModernBB has not yet been installed.',
...
'Description'					=>	'Unfortunately no one can be told what ModernBB is - you have to see it for yourself.',
...
'Info 1'						=>	'Please enter the requested information in order to setup your database for ModernBB. You must know all the information asked for before proceeding with the installation.',
'Select database'				=>	'Select your database type',
'Info 2'						=>	'ModernBB currently supports MySQL, PostgreSQL and SQLite. If your database of choice is missing from the drop-down menu below, it means this PHP environment does not have support for that particular database. More information regarding support for particular versions of each database can be found in the FAQ.',
'Dual MySQL'					=>	'ModernBB has detected that your PHP environment supports two different ways of communicating with MySQL. The two options are called standard and improved. If you are uncertain which one to use, start by trying improved and if that fails, try standard.',
'InnoDB'						=>	'ModernBB has detected that your MySQL server might support <a href="http://dev.mysql.com/doc/refman/5.0/en/innodb.html">InnoDB</a>. This would be a good choice if you are planning to run a large forum. If you are uncertain, it is recommended that you do not use InnoDB.',
...
'Info 4'						=>	'The name of the database that ModernBB will be installed into. The database must exist. For SQLite, this is the relative path to the database file. For security reasons, it is highly recommended to place the SQLite database in a private (not web accessible) directory whenever possible. If the SQLite database file does not exist, ModernBB will attempt to create it.',
...
'Info 6'						=>	'If you like, you can specify a table prefix. This way you can run multiple copies of ModernBB in the same database (example: foo_).',
'Table prefix'					=>	'Table prefix',
'Administration setup'			=>	'Administration setup',
'Info 7'						=>	'Please enter the requested information in order to setup an administrator for your ModernBB installation.',
...
'Info 11'						=>	'Please enter the requested information in order to setup your ModernBB board.',
...
'Enter base URL'				=>	'Enter the Base URL of your ModernBB installation',
'Info 14'						=>	'The URL (without trailing slash) of your ModernBB forum (example: http://forum.myhost.com or http://myhost.com/~myuser). This <strong>must</strong> be correct, otherwise, administrators and moderators will not be able to submit any forms. Please note that the preset value below is just an educated guess by ModernBB.',
...
'Info 17'						=>	'To finalize the installation, you need to click on the button below to download a file called config.php. You then need to upload this file to the root directory of your ModernBB installation.',
'Info 18'						=>	'Once you have uploaded config.php, ModernBB will be fully installed! At that point, you may <a href="index.php">go to the forum index</a>.',
