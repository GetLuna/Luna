...
'Update message'				=>	'Your ModernBB database is out-of-date and must be upgraded in order to continue. If you are the board administrator, please follow the instructions below to complete the upgrade.',
'Note'							=>	'Note:',
...
'Database password info'		=>	'To perform the database update please enter the database password with which ModernBB was installed. If you cannot remember, this is stored in your \'config.php\' file.',
...
'No charset conversion'			=>	'<strong>IMPORTANT!</strong> ModernBB has detected that this PHP environment does not have support for the encoding mechanisms required to do UTF-8 conversion from character sets other than ISO-8859-1. What this means is that if the current character set is not ISO-8859-1, ModernBB won\'t be able to convert your forum database to UTF-8 and you will have to do it manually. Instructions for doing manual charset conversion can be found in the update instructions.',
...
'Error info 1'					=>	'There was an error converting some users. This can occur when converting from ModernBB v1.2 if multiple users have registered with very similar usernames, for example "bob" and "böb".',
